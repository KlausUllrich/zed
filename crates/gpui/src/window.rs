...
impl Window {
    ...
    pub(crate) fn reuse_prepaint(&mut self, range: Range<PrepaintStateIndex>) {
        // Single shared implementation (Stage-3 "Form S"): the y-offset variant is
        // THE body; a zero offset is an exact identity for all positional math.
        // Keeping one body means upstream edits to the reuse path produce a visible
        // merge conflict instead of silently drifting against a fork-side copy.
        self.reuse_prepaint_with_y_offset(range, Pixels::ZERO);
    }
    ...
    pub(crate) fn reuse_paint(&mut self, range: Range<PaintIndex>) {
        // Single shared implementation (Stage-3 "Form S") — see `reuse_prepaint`.
        self.reuse_paint_with_y_offset(range, Pixels::ZERO);
    }
    ...
    ///
    /// This is the single shared implementation ("Form S", Stage-3 rebase):
    /// `reuse_prepaint` delegates here with `Pixels::ZERO`. A zero offset is an
    /// exact identity for every positional operation below, so the zero path
    /// behaves byte-for-byte like upstream's `reuse_prepaint`.
    ...
    ///
    /// This is the single shared implementation ("Form S", Stage-3 rebase):
    /// `reuse_paint` delegates here with `Pixels::ZERO`. The zero-offset path
    /// takes `Scene::replay` (bulk extend) rather than the per-primitive
    /// translate walk, so it stays as cheap as upstream's `reuse_paint`.
