...
use crate::{
    ...
    Overflow, Pixels, Point, PrepaintStateIndex, ScrollDelta, ScrollWheelEvent, Size, Style,
...
struct CachedItemScene {
    ...
    y_origin: Pixels,
    /// Range of prepaint state (hitboxes, text layouts, element states, dispatch nodes)
    /// captured during the frame this item was last rendered Fresh.
...
enum ItemRender {
    ...
    /// Skip full element lifecycle, replay cached scene with Y offset.
    Cached {
        scene_range: Range<usize>,
        cached_y: Pixels,
        prepaint_range: Range<PrepaintStateIndex>,
...
struct ItemLayout {
    ...
    y_origin: Pixels,
    /// Prepaint state range, captured during prepaint for both Fresh and Cached items.
    prepaint_range: Range<PrepaintStateIndex>,
}
            ...
            if scene_cache_hit {
                ...
                item_layouts.push_back(ItemLayout {
                    ...
                    render: ItemRender::Cached {
                        ...
                        prepaint_range: cached.prepaint_range.clone(),
                    },
                    size: measured_size,
                ...
                for item in &mut layout_response.item_layouts {
                    ...
                    match &mut item.render {
                        ItemRender::Fresh { element } => {
                            ...
                            item.prepaint_range = prepaint_start..prepaint_end;
                        ...
                        ItemRender::Cached { prepaint_range, .. } => {
                            // Reuse previous frame's prepaint state to keep text layouts,
                            // element states, and dispatch nodes alive during cached scroll.
                            let reuse_start = window.prepaint_index();
                            window.reuse_prepaint(prepaint_range.clone());
                            let reuse_end = window.prepaint_index();
                            item.prepaint_range = reuse_start..reuse_end;
                        }
                    }
...
impl Element for List {
    ...
    fn paint(
        ...
        let is_fully_visible = |y_origin: Pixels, height: Pixels| -> bool {
            y_origin >= bounds.origin.y
                && y_origin + height <= bounds.origin.y + bounds.size.height
        };
                    ...
                    ItemRender::Fresh { element } => {
                        ...
                        if debug_active {
                            debug_items.push(RenderCacheItemData {
                                ...
                                fully_visible: is_fully_visible(item.y_origin, item.size.height),
                    ...
                    ItemRender::Cached {
                        ...
                        if debug_active && !first_cached_debug_done {
                            ...
                            for (ptype, bb, ba, mb, ma, inter, surv, tt) in fates {
                                let (p_x, p_y, p_w, p_h) = ba;
                                let p_top = p_y;
                                let p_bottom = p_y + p_h;
                                let p_left = p_x;
                                let p_right = p_x + p_w;
        ...
        if debug_active {
            ...
            emit_render_cache_debug(&RenderCacheDebugEvent::Frame {
                summary: RenderCacheFrameData {
                    ...
                    fresh_scene_ops,
                    replay_scene_ops,
                    mask_expanded: caching_enabled && max_item_height > px(0.),
                ...
                for (index, range, y_origin, prepaint_range, item_height) in scene_updates {
                    if debug_active {
                        ...
                        debug_stores.push(CacheStoreData {
                            ...
                            scene_ops: ops,
                            y_origin: y_origin.0,
                            fully_visible: is_fully_visible(y_origin, item_height),
                        });
                    }
