...
pub struct TextureCacheDebugFrame {
    ...
    /// INV-V1 (backing destroyed pre-submit) violations observed during a
    /// *prior* frame. ≥ one-frame lag is intrinsic: V1 fires in the renderer
    /// after this debug frame has already been emitted, so the count is
    /// parked on the pool and surfaced on the next emit. The lag may extend
    /// beyond one frame during no-cache-region stretches, since
    /// `process_cache_regions` early-returns before emit when regions are
    /// empty — parked V1 counts then accumulate until cache work resumes.
...
pub(crate) struct TexturePool {
    ...
    /// in the next `process_cache_regions` debug emit that runs
    /// (≥ one-frame lag — usually one, but may extend during no-cache-region
    /// stretches since `process_cache_regions` early-returns before emit
    /// when `regions.is_empty()`. Accumulation is bounded by saturating_add,
    /// so worst case is u32::MAX of parked counts before the next emit
    /// drains). Analogous to `prev_composite_ms`. Surfaces fork-side races
    /// into the CS circuit breaker.
    #[cfg(feature = "texture-cache-debug")]
...
impl TexturePool {
    ...
    /// `take_pending_v1_violations` on the next debug-emit pass. During
    /// stretches with no cache regions the lag extends until cache work
    /// resumes (effectively fine since caching is off anyway).
    #[cfg(feature = "texture-cache-debug")]
