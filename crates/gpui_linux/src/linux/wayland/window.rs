...
impl PlatformWindow for WaylandWindow {
    ...
    fn completed_frame(&self) {
        ...
        // Register for presentation feedback (wp_presentation_time). The compositor
        // will fire Presented with the actual VSync timestamp, enabling accurate
        // animation dt computation. The feedback binds to the surface's NEXT content
        // update — either the bare commit below, or (when the renderer already
        // presented a buffer this frame) the renderer's own commit, which carries
        // the truer presented-frame timestamp.
